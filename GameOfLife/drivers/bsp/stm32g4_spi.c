...
/*
 * Ce module permet d'Initialiser un bus SPI, Recevoir et Transmettre des donn�es via ce SPI.
 *
 ...
 *
 * 		/!\ PF1 n'est accessible que si SB11 est soud� et pas SB10 (petits pads � souder/relier sur la carte stm32g431) /!\
 *
 */
...
/**
 * @brief Cette fonction initialise le bus SPI en fonction des 3 param�tres
 ...
 * @param SPI_bp: Le Baud Rate prescaler utilis� pour configurer la clock SCK0.
...
void SPI_GPIO_FULLDUPLEX_config(SPI_ID_e SPI_id){
	switch (SPI_id) {
		...
		case SPI2_ID:
		    ...
		    PF1-OSC_OUT ------> SPI2_SCK /!\ PF1 n'est accessible que si SB11 et pas SB10 (petits pads � souder/relier sur la carte stm32g431)
		    PA10     	------> SPI2_MISO
		    PA11     	------> SPI2_MOSI
		    */
...
/**
 * @brief Cette fonction sert � recevoir plusieurs donn�es sur l'un des bus SPI.
 * @param SPIx: le SPI � lire.
 * @param *data: la variable qui va stocker les donn�es lues.
 * @param count: le nombre de donn�es � recevoir.
 */
...
/**
 * @brief Cette fonction sert � envoyer plusieurs donn�es sur l'un des bus SPI.
...
/*
 * @brief Cette fonction sert � r�gler la taille d'une donn�e
 * @param SPIx le SPI dont on veut r�gler la taille des donn�e.
...
/**
 * @author louisz (portage spi clubrobot)
 * @brief Permet de r�gler le baudRate d'un SPI.
